...
public sealed class MemberEditorForm : Form
{
    ...
    {
        _chapter.DisplayMember = nameof(MemberChapterChoice.Text);
        var choices = new List<MemberChapterChoice> { new(null, "No Chapter Assigned") };
        choices.AddRange(new ChapterRepository().GetAll().Select(chapter => new MemberChapterChoice(chapter.ChapterID, chapter.ChapterName)));
        _chapter.DataSource = choices;
    ...
    private void LoadMember(long id)
    {
        ...
        var chapterIndex = _chapter.Items.Cast<MemberChapterChoice>()
            .Select((choice, index) => new { choice, index })
            .FirstOrDefault(item => item.choice.ChapterID == member.ChapterID)?.index ?? 0;
        _chapter.SelectedIndex = chapterIndex;
        _dirty = false;
    }
    ...
    private void Save(Control button)
    {
        ...
        if (address.Length == 0 || _status.SelectedItem == null)
        {
        ...
        try
        {
            ...
            var member = new Member
            {
                ...
                ChapterID = (_chapter.SelectedItem as MemberChapterChoice)?.ChapterID
            };
    ...
    private sealed record MemberChapterChoice(long? ChapterID, string Text);
